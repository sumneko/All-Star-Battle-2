...
globals
    ...
    private constant integer BONUS_TYPES = 8
...
private function DisplayAllAbilityId takes nothing returns nothing
    ...
    private function UnitClearBonus takes unit u, integer bonusType returns nothing
        ...
        loop
            ...
            set i=i-1
        endloop
        call YDWESaveIntegerByInteger(YDWEH2I(u),StringHash("bonusType")+bonusType,0)
    endfunction
    ...
    private function SetUnitMaxState takes unit u,integer bonusType,real value returns boolean
        ...
        call YDWESaveIntegerByInteger(YDWEH2I(u),StringHash("bonusType")+bonusType,v)
        if v>0 then
            set o=3
          elseif v<0 then
            set o=2
...
private function UnitSetBonus takes unit u, integer bonusType, integer ammount returns boolean
    ...
    call YDWESaveIntegerByInteger(YDWEH2I(u),StringHash("bonusType")+bonusType,ammount)
    if ammount < 0 then
...
private function UnitGetBonus takes unit u, integer bonusType returns integer
    return YDWEGetIntegerByInteger(YDWEH2I(u),StringHash("bonusType")+bonusType)
endfunction
...
private function InitializeAbilitys takes nothing returns nothing
    ...
    loop
		...
		loop
			exitwhen x > 11
			call SetPlayerAbilityAvailable(Player(x), BonusAbilitys[i], false)
			set x = x + 1
		endloop
        set i=i+1
        set m=m+1
